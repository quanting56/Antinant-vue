        ...
        <section class="content" v-if="currentTab" :key="currentTab.id">
          ...
          >
            ...
            >
              ...
              <!-- 大項程式碼 -->
              <div
                v-if="list.listCode.htmlCode || list.listCode.jsCode || list.listCode.tsCode || list.listCode.vueCode"
                class="code"
              >
                <template v-if="list.listCode.htmlCode">
                  <!-- html code: -->
                  <pre><code class="language-html">{{ list.listCode.htmlCode }}</code></pre>
                </template>

                <template v-if="list.listCode.jsCode">
                  <!-- javascript code: -->
                  <pre><code class="language-javascript">{{ list.listCode.jsCode }}</code></pre>
                </template>

                <template v-if="list.listCode.tsCode">
                  <!-- typescript code: -->
                  <pre><code class="language-typescript">{{ list.listCode.tsCode }}</code></pre>
                </template>

                <template v-if="list.listCode.vueCode">
                  Vue 3 SFC（使用Composition API）這樣寫：
                  <pre><code class="language-html">{{ list.listCode.vueCode }}</code></pre>
                </template>
              </div>
              ...
              <ol v-if="list.listDetails.length && list.listDetails[0].detailTitle">
                ...
                >
                  ...
                  <!-- 小項程式碼 -->
                  <div
                    v-if="detail.detailCode.htmlCode || detail.detailCode.jsCode || detail.detailCode.tsCode || detail.detailCode.vueCode"
                    class="code"
                  >
                    <template v-if="detail.detailCode.htmlCode">
                      <!-- html code: -->
                      <pre><code class="language-html">{{ detail.detailCode.htmlCode }}</code></pre>
                    </template>

                    <template v-if="detail.detailCode.jsCode">
                      <!-- javascript code: -->
                      <pre><code class="language-javascript">{{ detail.detailCode.jsCode }}</code></pre>
                    </template>

                    <template v-if="detail.detailCode.tsCode">
                      <!-- typescript code: -->
                      <pre><code class="language-typescript">{{ detail.detailCode.tsCode }}</code></pre>
                    </template>

                    <template v-if="detail.detailCode.vueCode">
                      Vue 3 SFC（使用Composition API）這樣寫：
                      <pre><code class="language-html">{{ detail.detailCode.vueCode }}</code></pre>
                    </template>
                  </div>
                </li>
              </ol>
...
// import 資料進來渲染
import { vueI18nNoteViewTabs } from "../../data/web-note-view/vue-i18n-note-view/vue-i18n-note-view-tabs";
const tabs = vueI18nNoteViewTabs;
